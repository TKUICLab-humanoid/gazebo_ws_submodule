/**
 * @file  src/qt_ros_pkg/ClassEample/classtestaa.cpp
 *
 * @brief Class Task_A
 *
 * @date 2015/10/16
...
void WalkingCycle::singlestepfunction(int walking_mode)
{
    ...
    if (walking_mode == 0 || walking_mode == 4 || walking_mode == 5)
    {
...
//parameterinfo->parameters.system_start = clock();
    ...
    //ROS_INFO("%d  %d",parameterinfo->complan.time_point_,parameterinfo->complan.sample_point_);  
...
void WalkingCycle::singlestepwalkingprocess()
{
    ...
    {
        case StartStep:
            ROS_INFO("StartStep, %d", parameterinfo->counter);
            ...
            break;
        case FirstStep:
            ROS_INFO("FirstStep, %d", parameterinfo->counter);
            ...
            break;
        case Repeat:
            ROS_INFO("Repeat, %d", parameterinfo->counter);
            //statetext = L"Repeat";
            if (parameterinfo->X > forwardCounter_ )
            {
                parameterinfo->XUpdate = forwardValue_;
            ...
            break;
        case StopStep:
            ROS_INFO("StopStep, %d", parameterinfo->counter);
...
void WalkingCycle::continuouswalkingprocess()
{
    ...
    switch (parameterinfo->complan.walking_state)
    {
        ...
        case MarkTimeStep:
            ROS_INFO("MarkTimeStep, %d", parameterinfo->counter);
            //        // Load Mark Time Parameters
            if(!parameterinfo->IsParametersLoad)
            {
                parameterinfo->IsParametersLoad = true;
        ...
        case ForwardStep:
            ROS_INFO("ForwardStep, %d", parameterinfo->counter);
            if(!parameterinfo->IsParametersLoad)
            {
                parameterinfo->IsParametersLoad = true;
            }
        ...
        case BackwardStep:
            ROS_INFO("BackwardStep, %d", parameterinfo->counter);
        ...
        case StopStep:
            ...
            if((parameterinfo->complan.time_point_ % parameterinfo->parameters.Period_T) == 0 && IsStop)// if(Times == 1)  JJ
            {
                parameterinfo->complan.walking_state = StopStep;
                parameterinfo->complan.walking_stop = true;
                parameterinfo->IsParametersLoad = false;
    ...
    if(parameterinfo->complan.time_point_ % (parameterinfo->parameters.Period_T/2) == 0 && !parameterinfo->complan.isfirststep)
        {
            parameterinfo->NowStep.data++;
            ROS_INFO("parameterinfo->NowStep.data: %d", parameterinfo->NowStep.data);
        }
}
...
void WalkingCycle::wosccontinuouswalkingprocess()
{
    ...
    case ForwardStep:
        ...
        if ( parameterinfo->complan.time_point_ % parameterinfo->parameters.Period_T ==  parameterinfo->parameters.Period_T *3 / 4)
        {
            ...
            if (parameterinfo->XUpdate == 0) {
                ...
                //ROS_INFO("sample_point_ = %d",parameterinfo->complan.sample_point_);
            }
        }
        break;
    case BackwardStep:
    ...
    parameterinfo->counter++;
    //ROS_INFO("value !!!!  [%f %f %f %f    %d]", parameterinfo->XUpdate,parameterinfo->YUpdate,parameterinfo->ZUpdate,parameterinfo->THTAUpdate,parameterinfo->counter);
    //ROS_INFO("%d\n",parameterinfo->counter);
