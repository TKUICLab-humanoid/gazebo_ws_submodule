...
int main(int argc, char** argv)
{
  ...
  ros::Publisher path_pub = node.advertise<nav_msgs::Path>("/trajectory/left_bottom_foot_fake_link",10);
  ...
  while (node.ok())
  {
    ...
    try
    {
      listener.lookupTransform("/dummy", "/left_bottom_foot_fake_link", ros::Time(0), transform);
    }
    catch (tf::TransformException ex)
    {
      ROS_ERROR("%s",ex.what());
