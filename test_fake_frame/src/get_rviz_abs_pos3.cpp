...
int main(int argc, char** argv)
{
  ...
  ros::Publisher path_pub = node.advertise<nav_msgs::Path>("/trajectory/right_bottom_foot_fake_link",10);
  ...
  while (node.ok())
  {
    ...
    try
    {
      listener.lookupTransform("/dummy", "/right_bottom_foot_fake_link", ros::Time(0), transform);
    }
    catch (tf::TransformException ex)
    {
      ROS_ERROR("%s",ex.what());
    ...
    if((float)transform.getOrigin().x() != old_x || (float)transform.getOrigin().y() != old_y || (float)transform.getOrigin().z() != old_z)
    {
      ...
      this_pose_stamped.pose.position.y = (float)transform.getOrigin().y();
